...
function cal() {
  ...
  du -sh /home/sftcwl/apps/* | sort -hr
  # 报警时同时列举文件大小超过1G的工程目录
  CONTENT=$(du -sh /home/sftcwl/apps/* | sort -hr | awk '$1~/G/ && $1+0>'${LIMIT_DISK_SIZE}' {print $1, $2}')
}
# 判断是否需要报警
function alarm() {
...
function noticeByWechat() {
  curl 'https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=13df63e2-32f0-4f97-b06a-6ab1460ce3cc' \
